        ...
        match self {
            ...
            Statement::Delete {
                ...
                if table_name.0.len() == 1 {
                    ctx.add_table(table_name.0.first().unwrap().value.clone(), String::from(""));
                } else {
                    ctx.add_table(table_name.0.last().unwrap().value.clone(), String::from(""));
                }
...
#[cfg(test)]
mod tests {
    use crate::handler::parser::sql::{SelectStatementContext, SQLStatementContext};
    ...
    fn test_analyse() {
        ...
        let sql = "delete from table_1 where cl = ? and exists (select 1 from table_2 t2 where t2.c5 = c5)";
