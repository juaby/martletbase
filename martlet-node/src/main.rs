...
async fn main() -> Result<(), Box<dyn Error>> {
    let matches = App::new("MartletBase")
        .version("0.1.0")
        .author("AlphaPo")
        .about("A Distributed Database based on MySQL")
    ...
    let config_path = matches.value_of("config").unwrap_or("./martlet-node/etc/node.toml");
